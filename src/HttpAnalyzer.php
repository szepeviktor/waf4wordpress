...
final class HttpAnalyzer
{
    ...
    public function __construct()
    {
        ...
        if (php_sapi_name() === 'cli' || (defined('WP_INSTALLING') && WP_INSTALLING)) {
            return;
        }
    ...
    private function read_constants()
    {
        ...
        if (defined('W4WP_INSTANT') && W4WP_INSTANT === false) {
            $this->instant_trigger = false;
        }
        ...
        if (defined('W4WP_PROXY_HOME_URL')) {
            ...
            if (substr($_SERVER['REQUEST_URI'], 0, $home_url_length) === W4WP_PROXY_HOME_URL) {
     ...
     *
     * @return string|bool Attack type or false.
     */
    private function check()
    {
        ...
        $request_query = $_SERVER['QUERY_STRING'] ?? parse_url($this->relative_request_uri, PHP_URL_QUERY);
        ...
        if (
            ...
            // Sample conditions.
            && $request_method === 'POST'
            && strpos($request_path, '/customer/account/createpost') !== false
        ...
        if (defined('XMLRPC_REQUEST') && XMLRPC_REQUEST) {
            $this->is_xmlrpc = true;
        } elseif (strpos($request_path, $this->login_url) !== false) {
            $this->is_login = true;
        } elseif (strpos($request_path, $this->rest_url_prefix) !== false) {
            $this->is_rest = true;
        }

        // Block non-static requests from CDN but allow robots.txt.
        if ($this->cdn_headers !== [] && $request_path !== '/robots.txt') {
        ...
        if (! $this->is_rest && $request_method === 'OPTIONS') {
        ...
        if (
            ! $this->is_login && ! $this->is_rest
            && in_array($request_method, $wp_methods, true) === false
        ) {
            return 'bad_request_http_method';
        }
        if ($this->is_login && in_array($request_method, $login_methods, true) === false) {
            return 'bad_request_login_http_method';
        }
        if ($this->is_rest && in_array($request_method, $rest_methods, true) === false) {
        ...
        if (
            ...
            || preg_match("/[^%:\/?\[\]@!$&'()*+,;=A-Za-z0-9._~-]/", $_SERVER['REQUEST_URI']) === 1
            || preg_match('/(http|https|data):/i', $request_path) === 1
        ) {
            $this->instant_trigger = false;
        ...
        if (
            stripos($this->relative_request_uri, 'robots.txt') !== false
            && $this->relative_request_uri !== '/robots.txt'
        ) {
        ...
        if (in_array($request_method, $write_methods, true) === false) {
        ...
        if (! empty($_FILES)) {
            foreach ($_FILES as $files) {
                ...
                foreach ($names as $key => $value) {
                    if (
                        stripos($value, '.php') !== false
                        || (
                            isset($types[$key])
                            && (
                                stripos($types[$key], 'php') !== false
                                || stripos($types[$key], 'application/x-shockwave-flash') !== false
                            )
                        )
                    ) {
        ...
        if (
            ...
            && ( empty($_SERVER['CONTENT_TYPE'])
                ...
                || (
                    $this->is_xmlrpc
                    && stripos($_SERVER['CONTENT_TYPE'], 'text/xml') !== 0
                )
                || (
                    stripos($_SERVER['CONTENT_TYPE'], 'application/x-www-form-urlencoded') !== 0
                    ...
                    && stripos($_SERVER['CONTENT_TYPE'], 'application/octet-stream') !== 0
                )
            )
        ) {
        ...
        if (! empty($_POST['log'])) {
            ...
            if (! $this->allow_two_capitals) {
                if (preg_match('/^[A-Z][a-z]+[A-Z][a-z]+$/', $username) === 1) {
        ...
        if ($request_query !== '') {
            ...
            if (isset($queries['action']) && $queries['action'] === 'postpass') {
                // wp-login/postpass.
                return false;
            }
        }
        ...
        if ($this->allow_ie8_login) {
            if (substr($user_agent, 0, 34) === 'Mozilla/4.0 (compatible; MSIE 8.0;') {
                // Allow IE8.
                return false;
            }
        }
        ...
        // Botnet user agents.
        if (preg_match($this->botnet_pattern, $user_agent) === 1) {
        ...
        if (! $this->allow_custom_registration) {
            if (
                ...
                || strpos($_SERVER['HTTP_COOKIE'], 'wordpress_test_cookie') === false
            ) {
        ...
        if (! $this->allow_custom_registration) {
            ...
            if (strpos($referer_path, $this->login_url) === false) {
...
',
    ...
    private function apache_request_headers()
    {
        ...
        foreach ($_SERVER as $name => $value) {
            ...
            } elseif (substr($name, 0, 8) === 'CONTENT_') {
                // RFC 3875
                $headers[$name] = $value;
            }
        }
    ...
    private function parse_query( $query_string )
    {
        ...
        foreach ($names_values_array as $name_value) {
            ...
            // Set field value
            $query[$name_value_array[0]] = $name_value_array[1] ?? '';
        }
     ...
     *
     * @return bool The client is IE 8, 9, 10 or 11.
     */
    private function is_ie( $ua )
    {

        return preg_match('/^Mozilla\/5\.0 \(Windows NT [0-9.]*;.* Trident\/7\.0; rv:11\.0\) like Gecko/', $ua) === 1
            || preg_match('/^Mozilla\/5\.0 \(compatible; MSIE 10\.0; Windows NT [0-9.]*;.* Trident\/6\.0/', $ua) === 1
            || preg_match('/^Mozilla\/5\.0 \(compatible; MSIE 9\.0; Windows NT [0-9.]*;.* Trident\/5\.0/', $ua) === 1
            || preg_match('/^Mozilla\/4\.0 \(compatible; MSIE 8\.0; Windows NT [0-9.]*;.* Trident\/4\.0/', $ua) === 1;
    }
    ...
    private function fix_opera_ua( $ua )
    {
        ...
        return (string)preg_replace('#(WUID=[0-9a-f]{32}; WTB=[0-9]+; )\1+#', '', $ua);
    }
    ...
    private function fix_facebook_dm( $ua )
    {
        ...
        return (string)preg_replace('#(FBDM/DisplayMetrics)\{([A-Za-z]+=[0-9.]+(, )?)+\}#', '\1', $ua);
    }
